...
clean:
	...
	rm -rf tower/tower/__pycache__ # remove compiled python directory
	rm -rf test/tower/__pycache__ # remove compiled python directory
...
format:
	...
	autopep8 --in-place --aggressive --experimental --max-line-length $(MAX_LINE) --recursive tower/
	...
	docformatter --in-place --wrap-summaries $(MAX_LINE) --wrap-descriptions $(MAX_LINE) \
		--pre-summary-newline --recursive tower/
	docformatter --in-place --wrap-summaries $(MAX_LINE) --wrap-descriptions $(MAX_LINE) \
		--pre-summary-newline --recursive test/

style: .python-check
	@echo "Check if 'tower' folder matches PEP-8 and PEP-257 guides"
	pydocstyle tower/
	pycodestyle tower/ --max-line-length=$(MAX_LINE) --ignore=E402
	@echo "Check if 'graph' folder matches PEP-8 and PEP-257 guides"
...
test: clean .python-check
	coverage run --source=tower,structures --omit=*"__init__.py" -m unittest discover -v
