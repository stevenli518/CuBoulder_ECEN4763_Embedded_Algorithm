...
clean:
	...
	rm -rf test/bellmanford/__pycache__ # remove compiled python directory
	rm -rf shortestpath/bellmanford/__pycache__ # remove compiled python directory
...
format:
	...
	autopep8 --in-place --aggressive --experimental --max-line-length $(MAX_LINE) --recursive shortestpath/
	...
	docformatter --in-place --wrap-summaries $(MAX_LINE) --wrap-descriptions $(MAX_LINE) \
		--pre-summary-newline --recursive shortestpath/
	docformatter --in-place --wrap-summaries $(MAX_LINE) --wrap-descriptions $(MAX_LINE) \
...
style: .python-check
	...
	@echo "Check if 'shortestpath' folder matches PEP-8 and PEP-257 guides"
	pydocstyle shortestpath/
	pycodestyle shortestpath/ --max-line-length=$(MAX_LINE)
	@echo "Run pylint"
	pylint test/
	pylint structures/
	pylint shortestpath/
...
test: clean .python-check
	coverage run --source=structures,shortestpath --omit=*"__init__.py" -m unittest -v
